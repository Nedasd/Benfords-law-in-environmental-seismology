...
if platform.system() == 'Darwin':
    ...
    dirs = "/home/qizhou/1projects/2confusion_matrix/1old_method/"### please change here
else:
...
def calculate_TP_TN (con_p, n_t, name):
    ...
    f = open(output_dir + "#confusion_matrix_2017_2019.txt", 'a')
...
con_positive_negative_space = dirs +"#P21_N1050_2017-2019(2017_ILL02).txt"
data_base = dirs + "all_2017-2019(2017_ILL02).txt"
# the testing parameter is stored here
df_p = pd.read_table(dirs + "round1_2017-2019.txt", delimiter=",", header=0)
...
for p in range(0, len(df_p) ):#len(df_p)
    ...
    store_space_output = output_dir + str(name) + ".txt"
    ...
    calculate_TP_TN(con_p=21, n_t=50, name=name)
    #print(str(name))

    e1 = datetime.datetime.now()
    print( (e1-s1).seconds )
    f = open("/home/qizhou/1projects/2confusion_matrix/1old_method/time.txt", 'a')
    f.write( str((e1-s1).seconds) + "\n")
    f.close()
